...
- **CTA wording**: every WhatsApp call-to-action says **"Falar com um advogado"** (header, hero, contact section, FAB). Do not introduce variants like "Vamos conversar" or "Falar agora pelo WhatsApp" without an explicit ask — the user standardized on this phrase to anchor the firm's identity. The Contact section h2 uses the imperative "Fale com um advogado." for headline rhythm.
- **No SLA promise**: response-time text ("24 horas", "1 dia útil", etc.) was deliberately removed from the site. Don't add it back without explicit instruction.
- **No contact form**: the contact section is intentionally WhatsApp-only — a centered headline + big animated WhatsApp button + horizontal channels strip (endereço / e-mail / horário / OAB) + a national-coverage pill at the bottom. Do not add a form back without explicit instruction; the user removed it deliberately to consolidate intake on WhatsApp.
